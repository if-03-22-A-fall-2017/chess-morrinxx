...
struct ChessPiece 	get_piece (ChessBoard chess_board, File file, Rank rank){
  if(chess_board[rank - 1][file - 'a'].is_occupied && (file >= 'a' && file <= 'h') && (rank > 0 && rank < 9)) return chess_board[rank - 1][file - 'a'].piece;
...
bool 	remove_piece (ChessBoard chess_board, File file, Rank rank){
  if(!chess_board[rank-1][file - 'a'].is_occupied) return false;
...
bool 	squares_share_diagonal (File s1_f, Rank s1_r, File s2_f, Rank s2_r){
  if(s1_f < 'a' || s1_f > 'h'||s2_f < 'a' || s2_f > 'h' || s1_r < 1 || s1_r > 8 ||s2_r < 1 || s2_r > 8) return false;
  if((s1_f - s2_f == s1_r - s2_r)||(-1 * (s1_f - s2_f) == s1_r - s2_r)) return true;
...
bool 	squares_share_pawns_move (enum PieceColor color, enum MoveType move, File s1_f, Rank s1_r, File s2_f, Rank s2_r){
  if ((s2_r - s1_r == 2 || s1_r - s2_r == 2) && is_move_from_base_line(color, s1_r)) {
    return true;
  }
  if ((color == White && s2_r - s1_r == 1 && s1_r != 1) || (color == Black && s1_r - s2_r == 1 && s1_r != 8)) {
    return true;
  }
  return false;
}
